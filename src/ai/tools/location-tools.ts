...
const NearbyPlacesInputSchema = z.object({
  query: z
    .string()
    .describe(
      'The type of place to search for (e.g., "coffee shop", "hardware store").'
    ),
  userLocation: z
    .string()
    .describe(
...
const NearbyPlacesOutputSchema = z.object({
  places: z.array(
    z.object({
      ...
      eta: z
        .string()
        .describe('The estimated time of arrival (e.g., "15 mins").'),
    })
  ),
});
...
export const findNearbyPlacesTool = ai.defineTool(
  {
    ...
    description:
      "Finds nearby places like businesses or landmarks based on a user's query and current location, and provides an estimated time of arrival (ETA) for each.",
  ...
  async (input) => {
    ...
    const { output } = await ai.generate({
      ...
      {
        "places": [
          { "name": "Philz Coffee", "address": "101 Forest Ave, Palo Alto, CA 94301", "eta": "8 mins" },
          { "name": "Verve Coffee Roasters", "address": "162 University Ave, Palo Alto, CA 94301", "eta": "12 mins" },
          { "name": "Blue Bottle Coffee", "address": "456 University Ave, Palo Alto, CA 94301", "eta": "10 mins" }
        ]
      }
